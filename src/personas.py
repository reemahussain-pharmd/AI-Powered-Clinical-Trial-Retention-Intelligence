"""
Patient Persona Classifier for Clinical Trial Retention.
...
strategies and intervention prioritisation, reflecting a patient-centric
...
PERSONA_DESCRIPTIONS = {
    ...
    "Polypharmacy High-Risk Patient": (
        "Patient on ≥8 medications with multiple comorbidities and prior adverse event history. "
...
def classify_persona(patient_features: pd.Series) -> Tuple[str, str]:
    """
    Classify a patient into a retention persona based on their feature profile.

    Persona assignment is hierarchical: the most clinically specific persona
    takes precedence. Long-distance rural patients are identified first since
    logistics alone can drive dropout regardless of clinical status.

    Args:
        patient_features: Series of patient feature values (single row).
    ...
    if medications >= 8 and comorbidities >= 4 and prior_ae == "yes":
        name = "Polypharmacy High-Risk Patient"
        return name, PERSONA_DESCRIPTIONS[name]

    # High Burden Elderly — comorbid older patient with visit fatigue risk
    if age > 65 and comorbidities >= 4 and medications >= 6:
        name = "High Burden Elderly Patient"
        return name, PERSONA_DESCRIPTIONS[name]
    ...
    scores = {
        "High Burden Elderly Patient": (age / 80) * 0.4 + (comorbidities / 8) * 0.3 + (medications / 12) * 0.3,
        "Young Mobile Professional": (1 if 25 <= age <= 45 else 0) * 0.4 + (visit_freq / 12) * 0.3 + (distance / 200) * 0.3,
        "Polypharmacy High-Risk Patient": (medications / 12) * 0.4 + (comorbidities / 8) * 0.3 + (1 if prior_ae == "yes" else 0) * 0.3,
